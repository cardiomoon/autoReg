---
title: "Statiastical tests in gaze"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Statiastical tests in gaze}
...
If you want to compare EF means between males and females in acs data with parametric method,
you have to compare the variances of two samples. If the variances of two groups are equal, the pooled variance is used to estimate the variance. Otherwise the Welch (or Satterthwaite) approximation to the degrees of freedom is used.
...
When method=3, perform the Shapiro-Wilk test or the Anderson-Daring test for normality(nortest::ad.test) to decide between normal or non-normal. If the number of cases are below 5000, Shapiro-Wilk test perfomed. If above 5000, Anderson-Daring test for normality performed.
...
The Dx column of acs data is diagnosis. It has three groups : Unstable Angina, NSTEMI and STEMI. You can make a table summarizing baseline characteristics among three groups. The parametric method cmparing means of three or more groups is ANOVA, whereas non-paramatric method is Kruskal-Wallis rank sum test.
...
## Statistical tests for categorial variables
...
- 2 : Perform chisq.test with continuity correction (default value)
...
If you want to perform chi-squared test without continuity correction, just set catMethod=1. This is the default metohd in SPSS.
...
You can seelct whether or not show total column.
